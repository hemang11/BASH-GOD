...
_BASH_GOD_CATALOG_DIR="${_BASH_GOD_CORE_DIR%/src}/catalog"
_BASH_GOD_VERSION='0.0.3.1'
...
# Requirements Schema 1 computes environment eligibility independently from
# catalog parsing. It is optional during staged rollout; the shared picker
# admits Schema-1 rows only when their fresh result is eligible, while static
# knowledge routes remain complete and inert.
if [ -r "$_BASH_GOD_CORE_DIR/eligibility.sh" ]; then
  # shellcheck source=eligibility.sh
  . "$_BASH_GOD_CORE_DIR/eligibility.sh" || return 1
fi
# shellcheck source=ui/art.sh
. "$_BASH_GOD_CORE_DIR/ui/art.sh" || return 1
# shellcheck source=ui/render.sh
. "$_BASH_GOD_CORE_DIR/ui/render.sh" || return 1

# Tree and search are separate concerns but share the validated catalog and style helpers above.
# shellcheck source=ui/tree.sh
. "$_BASH_GOD_CORE_DIR/ui/tree.sh" || return 1
...
# no @discover block.
if [ -r "$_BASH_GOD_CORE_DIR/ui/input.sh" ]; then
  # shellcheck source=ui/input.sh
  . "$_BASH_GOD_CORE_DIR/ui/input.sh" || return 1
fi
if [ -r "$_BASH_GOD_CORE_DIR/ui/menu.sh" ]; then
  # shellcheck source=ui/menu.sh
  . "$_BASH_GOD_CORE_DIR/ui/menu.sh" || return 1
fi
if [ -r "$_BASH_GOD_CORE_DIR/ui/tui.sh" ]; then
  # shellcheck source=ui/tui.sh
  . "$_BASH_GOD_CORE_DIR/ui/tui.sh" || return 1
fi
...
if [ -r "$_BASH_GOD_CORE_DIR/interaction.sh" ]; then
  # shellcheck source=interaction.sh
  . "$_BASH_GOD_CORE_DIR/interaction.sh" || return 1
fi
