...
# The package has a native terminal helper, so an update must name its target
# explicitly. Keep this bounded rather than treating an unfamiliar uname as a
# near-enough architecture.
_god_maintenance_platform() {
  local os arch
...
_god_maintenance_init_paths() {
  ...
  _god_maintenance_helper_dir="$_god_maintenance_prefix/libexec/bash-god"
...
_god_maintenance_install_update() {
  local version platform release_url archive archive_checksum installer installer_checksum dependency
  ...
  platform="$(_god_maintenance_platform)" || \
    _god_maintenance_die 'unsupported host platform; supported targets are darwin-amd64, darwin-arm64, linux-amd64, and linux-arm64'
  ...
  archive="$_god_maintenance_download_dir/bash-god-${version}-${platform}.tar.gz"
  archive_checksum="${archive}.sha256"
  installer="$_god_maintenance_download_dir/install-runtime.sh"
  installer_checksum="${installer}.sha256"

  _god_maintenance_download "$release_url/bash-god-${version}-${platform}.tar.gz" "$archive"
  _god_maintenance_download "$release_url/bash-god-${version}-${platform}.tar.gz.sha256" "$archive_checksum"
...
_god_maintenance_uninstall() {
  ...
  _god_maintenance_path_row "$_god_maintenance_helper_dir"
  ...
  _god_maintenance_remove_dir "$_god_maintenance_helper_dir"
  ...
  rmdir "$_god_maintenance_prefix/libexec" 2>/dev/null || true
