...
_god_validate_catalog() {
  LC_ALL=C awk -v catalog="$1" '
    ...
    # Requirements Schema 1 deliberately has a much smaller top-level
    # vocabulary than a command block. Service defaults are scalar only;
    # tools belong to the command that actually needs them.
    function validate_environment_row(line, count, parts, kind, subject, value, key) {
      count = split(line, parts, /\|/)
      if (count != 3 || trim(parts[1]) == "" || trim(parts[2]) == "" || trim(parts[3]) == "") {
        fail("invalid @environment row on line " NR "; expected KIND | SUBJECT | VALUE")
        return
      }
      ...
      value = trim(parts[3])
      key = kind SUBSEP subject
      environment_row_count++
      if (environment_seen[key]++) {
        fail("@environment repeats " kind " | " subject)
        return
      }

      if (kind == "os" && subject == "local") {
        if (value !~ /^(any|linux|darwin|freebsd)$/) {
          fail("@environment os | local must be any, linux, darwin, or freebsd")
        } else {
          environment_os_local = value
        }
      } else if (kind == "context" && subject == "execution") {
        if (value !~ /^(local|remote)$/) {
          fail("@environment context | execution must be local or remote")
        } else {
          environment_context = value
        }
      } else if (kind == "shell" && subject == "local") {
        if (value !~ /^(none|posix|bash)$/) {
          fail("@environment shell | local must be none, posix, or bash")
        } else {
          environment_shell_local = value
        }
      } else {
        fail("@environment accepts only os | local, context | execution, or shell | local defaults")
      }
    }

    # @requires rows are data, not a shell parser. In particular, the
    # validator never infers tools from @run: pipelines and command
    # substitutions stay explicit review decisions in catalog text.
    function validate_requires_row(line, count, parts, kind, subject, value, key, scope_parts, scope, name) {
      count = split(line, parts, /\|/)
      if (count != 3 || trim(parts[1]) == "" || trim(parts[2]) == "" || trim(parts[3]) == "") {
        fail("invalid @requires row on line " NR "; expected KIND | SUBJECT | VALUE")
        return
      }
      ...
      value = trim(parts[3])
      requires_row_count++
      key = record_serial SUBSEP kind SUBSEP subject

      if (kind == "os") {
        if (subject !~ /^(local|remote)$/ || value !~ /^(any|linux|darwin|freebsd)$/) {
          fail("@requires os must be local or remote with any, linux, darwin, or freebsd")
          return
        }
        if (requires_scalar_seen[key]++) fail("command \"" command_name "\" repeats @requires os | " subject)
        if (subject == "local") effective_os_local = value
        # `any` is a neutral constraint. It must not let a remote command
        # bypass the explicit-remote-fact boundary by pretending to require a
        # remote OS while asking for no information about it.
        else if (value != "any") requires_remote_fact = 1
      } else if (kind == "context") {
        if (subject != "execution" || value !~ /^(local|remote)$/) {
          fail("@requires context must be execution | local or remote")
          return
        }
        if (requires_scalar_seen[key]++) fail("command \"" command_name "\" repeats @requires context | execution")
        effective_context = value
      } else if (kind == "shell") {
        if (subject !~ /^(local|remote)$/ || value !~ /^(none|posix|bash)$/) {
          fail("@requires shell must be local or remote with none, posix, or bash")
          return
        }
        if (requires_scalar_seen[key]++) fail("command \"" command_name "\" repeats @requires shell | " subject)
        if (subject == "local") effective_shell_local = value
        # `none` is likewise neutral: it says no remote shell grammar is
        # needed, not that any remote environment has been established.
        else if (value != "none") requires_remote_fact = 1
      } else if (kind == "tool" || kind == "tool-version") {
        if (subject !~ /^(local|service|remote):[A-Za-z0-9][A-Za-z0-9._+-]*$/) {
          fail("@requires " kind " has invalid tool subject \"" subject "\"")
          return
        }
        split(subject, scope_parts, /:/)
        scope = scope_parts[1]
        name = scope_parts[2]
        if (scope == "service" && !seen_discover) {
          fail("@requires service:" name " requires an @discover catalog")
        }
        if (kind == "tool") {
          if (value !~ /^(present|gnu|bsd)$/) {
            fail("@requires tool " subject " must be present, gnu, or bsd")
            return
          }
          if (requires_tool_seen[record_serial SUBSEP subject]++) {
            fail("command \"" command_name "\" repeats @requires tool | " subject)
          }
        } else {
          if (value !~ /^(>=|>|=|<=|<)[0-9]+(\.[0-9]+)*$/) {
            fail("@requires tool-version " subject " must use >=, >, =, <=, or < with a dotted numeric version")
            return
          }
          requires_tool_version_seen[record_serial SUBSEP subject] = 1
        }
        # A remote tool requirement is meaningful evidence; a tool-version
        # row alone is not (and is separately required to have a tool row).
        if (scope == "remote" && kind == "tool") requires_remote_fact = 1
      } else {
        fail("unknown @requires kind \"" kind "\"; expected os, context, shell, tool, or tool-version")
      }
    }
    ...
    !in_command && $0 == "@discover" {
      ...
      if (seen_environment) fail("@discover must appear before @environment 1")
    ...
    !in_command && /^@execution([[:space:]]|$)/ {
      ...
      if (seen_environment) fail("@execution must appear before @environment 1")
    ...
    !in_command && /^@connection([[:space:]]|$)/ {
      ...
      if (seen_environment) fail("@connection must appear before @environment 1")
    ...
    !in_command && /^@synced[[:space:]]+/ {
      ...
      if (seen_environment) fail("@synced must appear before @environment 1")
    ...
    !in_command && /^@environment([[:space:]]|$)/ {
      if (group_count > 0) fail("@environment must appear before all groups")
      if (!seen_title || !seen_catalog_description) fail("@environment must appear after @title and catalog @description")
      if (seen_environment) fail("duplicate @environment")
      environment_schema = $0
      sub(/^@environment[[:space:]]*/, "", environment_schema)
      if (environment_schema != "1") fail("@environment must be exactly @environment 1")
      seen_environment = 1
      environment_row_count = 0
      environment_os_local = "any"
      environment_context = "local"
      environment_shell_local = "none"
      in_catalog_description = 0
      in_discover = 0
      in_environment = 1
      next
    ...
    /^@command[[:space:]]+/ {
      ...
      requires_remote_fact = 0
      effective_os_local = environment_os_local
      effective_context = environment_context
      effective_shell_local = environment_shell_local
      record_serial++
      field = ""
      next
    }
    ...
    in_command && /^@mode[[:space:]]+/ {
      if (has_requires) fail("command \"" command_name "\" has @mode after @requires")
    ...
    in_command && /^@risk[[:space:]]+/ {
      if (has_requires) fail("command \"" command_name "\" has @risk after @requires")
    ...
    in_command && /^@since[[:space:]]+/ {
      if (has_requires) fail("command \"" command_name "\" has @since after @requires")
    ...
    in_command && /^@until[[:space:]]+/ {
      if (has_requires) fail("command \"" command_name "\" has @until after @requires")
    ...
    in_command && /^@intent[[:space:]]+/ {
      if (has_requires) fail("command \"" command_name "\" has @intent after @requires")
    ...
    in_command && $0 == "@requires" {
      if (!seen_environment) fail("command \"" command_name "\" uses @requires without @environment 1")
      if (has_requires) fail("command \"" command_name "\" has duplicate @requires")
      if (!has_mode) fail("command \"" command_name "\" must declare @mode before @requires")
      if (seen_discover && !has_since) fail("command \"" command_name "\" must declare @since before @requires")
      if (has_description || has_run || has_params || has_optional || has_notes) {
        fail("command \"" command_name "\" must place @requires before description, run, params, optional, and notes")
      }
      has_requires = 1
      field = "requires"
      next
    ...
    $0 == "@end" {
      ...
      if (seen_environment && !has_requires) {
        fail("command \"" command_name "\" has no @requires; every Requirements Schema 1 command must declare its requirements")
      } else if (has_requires && !requires_row_count) {
        fail("command \"" command_name "\" has an empty @requires block")
      }
      ...
      if (has_requires && effective_context == "local" && requires_remote_fact) {
        fail("command \"" command_name "\" declares remote requirements while its effective context is local")
      }
      if (has_requires && effective_context == "remote" && !requires_remote_fact) {
        fail("command \"" command_name "\" has remote execution context but no meaningful remote OS, shell, or tool requirement")
      }
      if (has_requires) {
        for (tool_key in requires_tool_version_seen) {
          split(tool_key, tool_key_parts, SUBSEP)
          if (tool_key_parts[1] == record_serial && !(tool_key in requires_tool_seen)) {
            fail("command \"" command_name "\" has @requires tool-version without matching tool | " tool_key_parts[2])
          }
        }
      }
      in_command = 0
    ...
    in_command && field == "requires" && /[^[:space:]]/ {
      validate_requires_row($0)
    }
    ...
    !in_command && in_environment && /[^[:space:]]/ { validate_environment_row($0) }
    ...
    !in_command && /[^[:space:]]/ && !in_catalog_description && !in_discover && !in_environment {
    ...
    END {
      ...
      if (seen_environment && !environment_row_count) fail("@environment 1 has no default rows")
      if (previous_group != "" && group_command_count == 0) fail("group \"" previous_group "\" has no commands")
...
# _god_catalog_environment_schema FILE
#
# Prints `1` only when a catalog has explicitly opted into Requirements Schema
# 1. Schema-0 catalogs intentionally print nothing so existing browsing and
# execution behavior remains unchanged until R12/R13 migrate a whole service.
_god_catalog_environment_schema() {
  LC_ALL=C awk '
    /^@environment[[:space:]]+1$/ { print "1"; exit }
    /^@group[[:space:]]+/ { exit }
  ' "$1"
}

# _god_catalog_environment_export FILE
#
# Emits the service defaults for one opt-in catalog as tagged tab-separated
# data. Command exports carry their own REQUIRE rows; this separate export
# makes the service/command boundary explicit without a service-name branch.
#
#   SCHEMA\t1
#   DEFAULT\tKIND\tSUBJECT\tVALUE
_god_catalog_environment_export() {
  LC_ALL=C awk '
    function trim(value) {
      sub(/^[[:space:]]+/, "", value)
      sub(/[[:space:]]+$/, "", value)
    ...
    /^@environment[[:space:]]+1$/ { in_environment = 1; print "SCHEMA\t1"; next }
    in_environment && /^@/ { exit }
    in_environment && /[^[:space:]]/ {
      count = split($0, parts, /\|/)
      if (count == 3) print "DEFAULT\t" trim(parts[1]) "\t" trim(parts[2]) "\t" trim(parts[3])
    }
  ' "$1"
}
...
#   REQUIRE\t<kind>\t<subject>\t<value>
#                              (one row per Schema-1 @requires entry)
...
_god_catalog_command_export() {
  LC_ALL=C awk \
    ...
    selected && $0 == "@requires" { field = "requires"; next }
    ...
    selected && field == "requires" && /[^[:space:]]/ {
      columns = split($0, parts, /\|/)
      if (columns == 3) print "REQUIRE\t" trim(parts[1]) "\t" trim(parts[2]) "\t" trim(parts[3])
      next
    }
...
# _god_catalog_command_service_tool FILE GROUP ENTRY_INDEX
#
# Schema-1 records can name the exact executable that must exist in the
# resolved service directory. This is deliberately record metadata rather
# than a service-name rule: a Mongo legacy-shell row can require `mongo` while
# a modern row requires `mongosh`, and a Kafka sibling command can require its
# own script. A record with no service-scoped tool returns nothing.
_god_catalog_command_service_tool() {
  _god_catalog_command_export "$1" "$2" "$3" | LC_ALL=C awk -F '\t' '
    $1 == "REQUIRE" && $2 == "tool" && $3 ~ /^service:/ {
      sub(/^service:/, "", $3)
      print $3
      exit
    }
