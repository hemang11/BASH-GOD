...
_god_search_catalog() {
  ...
  GOD_SEARCH_PATTERN="$pattern" GOD_SEARCH_MODE="$search_mode" LC_ALL=C awk \
    ...
    function flush() {
      ...
      if (score >= 0) {
        printf "%d\t%d\t%s\t%s\t%d\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", \
          score, match_coverage, service, group, entry_number, command_title, run_command, risk, since, until_version, intent, environment_schema, mode
        hits++
      }
      in_command = 0
    }
...
_god_search_apply_policy() {
  ...
  LC_ALL=C awk -F "$tab" -v OFS="$tab" -v version_map="$version_map" -v all_versions="$all_versions" '
...

    {
      ...
      # Schema-1 requirements can make an otherwise newer in-range variant
      # ineligible locally. Keep that family intact until interaction.sh has
      # evaluated each record, rather than hiding an eligible sibling early.
      if (all_versions != "1" && intent != "" && numeric_version && schema != "1") {
...
$catalog_output"
  ...
  rich_mode=0
  if [ "$search_view" = "list" ] && \
     [ -n "$(type -t _god_interaction_available 2>/dev/null)" ] && \
     _god_interaction_available "$sorted_results" "$tab"; then
    rich_mode=1
  fi
  ...
  case "$search_view" in
    ...
    *)
      if [ "$rich_mode" = 1 ]; then
        _god_interaction_offer_search_results "$sorted_results" "$tab" "$pattern" "$search_title RESULTS" "$search_label"
        search_status=$?
        if [ "$search_status" -eq "${_GOD_TUI_STATIC_FALLBACK_STATUS:-125}" ] || \
           [ "$search_status" -eq "${_GOD_INTERACTION_NO_CANDIDATE_STATUS:-126}" ]; then
          _god_banner "$search_title RESULTS" "$search_label"
          _god_render_search_list "$sorted_results" "$tab" "$search_mode"
          if [ "$search_status" -eq "${_GOD_INTERACTION_NO_CANDIDATE_STATUS:-126}" ]; then
            printf '\n%s  No reviewed command is executable in this environment.%s\n' \
              "$_GOD_DIM" "$_GOD_RESET"
            [ -z "${_god_interaction_no_candidate_reason:-}" ] || \
              printf '%s  %s%s\n' "$_GOD_DIM" "$_god_interaction_no_candidate_reason" "$_GOD_RESET"
          fi
          return 0
        fi
