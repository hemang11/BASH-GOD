...
2. **Decide per affected record**:
   ...
   - **Both an old and a new form are valid for some version range** — give both records the same
     ...
     near-duplicates. See `catalog/AGENTS.md`'s Execution Metadata section for the exact mechanics
...
5. **Validate and test**:
   ...
   intended. `src/discover.sh`'s cache can be primed directly for this without a real install:
...
- Not writing code. `src/discover.sh`, `src/resolve.sh`, `src/interaction.sh`, and `src/execute.sh` are generic and version-neutral;
