...
restore, and native tool help. Commands are displayed as inert text and are never executed by
BASH_GOD. Replace every placeholder before using a command; no example contains credentials.
...
@notes
Do not add a password to the URI or command line; use the hidden interactive prompt.
@end
...
DATABASE | <database> | Initial database selected after connecting
@notes
The mongo shell is legacy and is absent from newer MongoDB packages; prefer mongosh when it is available.
@end
...
rs.status()
@notes
Run inside mongosh while connected to a replica-set member.
@end
...
@notes
Run the preview row first. Existing matching documents are not removed unless an explicit destructive option is added.
@end
...
PATH | <dump_directory> | Root directory created by mongodump
@notes
Current database tools prefer --nsInclude over the deprecated --db and --collection restore flags for directories and archives.
@end
...
@notes
Use the same --archive, --gzip, and --nsInclude options with --dryRun --verbose to preview the archive first.
@end
