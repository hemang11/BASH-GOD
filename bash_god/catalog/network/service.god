...
@description
Curated network knowledge for interfaces, routes, listening ports, DNS, connectivity, HTTP, SSH,
and read-only Route 53 discovery. Commands are displayed as inert text and are never executed by
BASH_GOD.
...
@mode MODERN
@description
Displays the account, principal ARN, and user ID used by subsequent read-only Route 53 commands.
@run
