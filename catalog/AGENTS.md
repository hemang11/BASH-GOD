...
engine (`src/discover.sh`, `src/resolve.sh`, `src/interaction.sh`, `src/execute.sh`); it never means writing new code that runs
...
8. Preserve BASH_GOD's inert-catalog boundary when adding or editing a catalog: never write code that
   executes a command yourself. The one generic engine may run the reviewed selected `@run` command
   only for a catalog declaring `@discover` or `@execution PATH`; catalog changes are text, not code.
   The shared picker owns browsing, then releases the terminal before shared editing, prompts, or
   child execution; no catalog may bypass that lifecycle.
...
They execute only `src/maintenance.sh`, never an `@run` value. Automatic update checks are
...
1. Open `catalog/<service>/service.god`.
...
@requires
tool | service:<reviewed_client> | present
@description
...
```text
catalog/example-service/service.god
```

The `service.god` content begins like this:

```text
@title Example service commands

@description
Curated local service inspection commands. Static views are inert; executable search runs only the
reviewed selected command after the operator confirms it.
...
@command Check a Linux systemd service status
@mode LOCAL
@requires
os | local | linux
tool | local:systemctl | present
@description
Shows whether one local systemd unit is running and displays recent status information.
@run
...
```text
god example-service
god example-service service
```

Do not register the new service manually in `core.sh`. The parent directory name becomes the route,
so `catalog/example-service/service.god` becomes `god example-service`. Automatic discovery should make it appear in
...
| top-level `@environment 1` | Required for executable production catalogs, once per file | Opts the entire catalog into Requirements Schema 1. It follows top-level execution, connection, and sync metadata, precedes every group, and makes `@requires` mandatory for every record. |
...
| `@requires` | Exactly once per record in a Schema-1 catalog | A non-empty requirement block after `@mode` and optional risk/version/intent metadata, before description/run fields. Each row is `KIND | SUBJECT | VALUE`. |
...
`LEGACY-ZK` as `[LEGACY]`. `LOCAL` remains version-neutral. In Schema 1 it uses its declared local
requirements and retains a discovery directory only when it explicitly declares `service:NAME`.
Do not invent another mode without updating validation, rendering, tests, and this guide together.

## Requirements Schema 1 (`@environment 1` and `@requires`)

Requirements Schema 1 is the whole-catalog contract for every current
executable production catalog. It adds bounded environment evidence; it does
not add a platform command compiler, a shell parser, a service-specific
exception, or permission to run an operational command. A new executable
catalog must annotate every record before it is added. Schema 0 remains
parser-compatible only for historical and test-fixture coverage.

`@environment 1` has only scalar local defaults:
...
Accepted values are `any`, `linux`, `darwin`, or `freebsd` for
`os | local`; `local` or `remote` for
`context | execution`; and `none`, `posix`, or `bash` for
`shell | local`. Omit a row to retain its neutral default.

Every Schema-1 record then declares one non-empty `@requires` block:

~~~text
@requires
tool | local:jq | present
tool | local:date | gnu
tool-version | local:date | >=9.0
~~~

- `os` and `shell` use a `local` or `remote` subject. Remote facts
  require remote execution context and stay unknown unless a future approved
  fact source explicitly supplies them; BASH_GOD never SSHes just to learn
  them.
- `context | execution | local|remote` overrides the service default for
  that record.
- `tool` subjects are bare `local:NAME`, `service:NAME`, or
  `remote:NAME`; values are `present`, `gnu`, or `bsd`.
  `service:NAME` is valid only in a catalog with `@discover` and means that
  exact executable in the resolved directory—not the selected probe or any
  sibling tool.
- `tool-version` uses the same subject and one dotted numeric comparator:
  `>=`, `>`, `=`, `<=`, or `<`. A matching `tool` row is required.

Name every external executable in a reviewed pipeline or command substitution.
Do not infer tools from `@run`, embed fallback syntax, or translate GNU and
BSD flags. Model genuinely different native spellings as separate reviewed
records linked with `@intent`.

The shared evaluator returns only `eligible`, `ineligible`, or
`unknown` from declared requirements and bounded facts. The existing shared
picker admits a Schema-1 row only when it is eligible; ineligible and unknown
rows remain available in static knowledge views. Static rendering does not
collect facts merely to add row labels. Read the
[`catalog contract architecture`](../docs/architecture/catalog-contract-architecture.md)
before changing the vocabulary or opting a service in.
...
generic engine to resolve values and run a confirmed command (`src/discover.sh`, `src/resolve.sh`,
`src/interaction.sh`, `src/execute.sh`). Requirements Schema 1 evaluates
declared facts in `src/eligibility.sh`; the shared interaction coordinator
uses only an eligible Schema-1 result to form its executable candidate set.
Nothing service-specific belongs in either layer.
...
Rich keyboard parsing is owned by the compiled `god-tui` helper. A missing,
mismatched, or unsupported helper must leave the static fallback available.

When a native release changes what a record needs, see `docs/service-sync.md` for the
...
1. initialization/dispatch in `src/core.sh`, catalog discovery/validation/readers in
   `src/catalog.sh`, bare-TTY artwork in `src/ui/art.sh`, normal views in
   `src/ui/render.sh`, search in `src/search.sh`, hierarchy rendering in `src/ui/tree.sh`,
   interaction coordination in `src/interaction.sh`, the retained basic maintenance selector in
   `src/ui/menu.sh`, terminal input in `src/ui/input.sh`, the helper adapter in `src/ui/tui.sh`,
   the picker in `cmd/god-tui/` and `internal/tui/`, service-path discovery in `src/discover.sh`,
   bounded eligibility facts and decisions in `src/eligibility.sh`, value resolution in
   `src/resolve.sh`, or confirmed execution in `src/execute.sh`,
   according to ownership;
2. smoke coverage in `tests/smoke.sh`;
3. `docs/architecture/bash-god-knowledge-base-architecture.md`;
...
- execution is generic for catalogs declaring `@discover` or `@execution PATH`; every executable
  ...
  A discoverable service without a fresh resolved path, a catalog with neither marker, and a runtime
  without a protocol-compatible `god-tui` helper keep the static, copy-ready result view.
