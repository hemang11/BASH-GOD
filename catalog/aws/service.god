...
tool | service:aws | present
@description
Lists the EC2 instances visible to the current AWS identity, including their state, networking, tags, and configuration.
@run
aws ec2 describe-instances --output table
@params
--output | table | Render the EC2 instance information in a human-readable table
@optional
--region | <aws_region> | Region to inspect when it differs from the configured default
@notes
EC2 instances are regional, so an empty result may mean the configured region is not the one containing them.
@end

@command Show one EC2 instance
@mode MODERN
@since 1.16.12
@requires
tool | service:aws | present
@description
Shows the state, networking, tags, and configuration of one EC2 instance selected by its instance ID.
@run
aws ec2 describe-instances --instance-ids <instance_id> --output table
@params
--instance-ids | <instance_id> | EC2 instance to inspect
--output | table | Render the EC2 instance information in a human-readable table
@optional
--region | <aws_region> | Region to inspect when it differs from the configured default
@notes
EC2 instance IDs are regional; use the region containing the instance.
@end
...
@requires
tool | service:aws | present
tool | local:session-manager-plugin | present
@description
