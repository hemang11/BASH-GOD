...
restore, and native tool help. Modern mongosh and legacy mongo records name their exact shell;
discovery can locate a directory through either installed client, but never rewrites one shell into
the other. Shell expressions are launched through the reviewed client with --eval, so the picker
always presents an executable command. The detected client version applies only to direct shell
command-line records; it does not establish MongoDB server or MongoDB Database Tools compatibility.
Replace every placeholder before using a command; no example contains credentials.

@discover
probe | mongosh | Preferred modern MongoDB Shell executable
probe | mongo | Legacy MongoDB shell used to locate a client directory when mongosh is absent
...
@since 0.0
@requires
tool | local:mongorestore | present
@description
