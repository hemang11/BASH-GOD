...
catalog="$project_dir/catalog/mongo/service.god"
...
. "$project_dir/src/interaction.sh"
modern_model="$(_god_resolve_command mongo "$catalog" connect 1 "$fixture_bin" '')"
legacy_model="$(_god_resolve_command mongo "$catalog" connect 4 "$fixture_bin" '')"
modern_service_tool="$(_god_catalog_command_service_tool "$catalog" connect 1)"
legacy_service_tool="$(_god_catalog_command_service_tool "$catalog" connect 4)"
if printf '%s\n' "$modern_model" | LC_ALL=C grep -Fq 'DISPLAY	mongosh "mongodb://' && \
   printf '%s\n' "$legacy_model" | LC_ALL=C grep -Fq "$fixture_bin/mongo --host <host> --port 27017" && \
   [ "$modern_service_tool" = mongosh ] && [ "$legacy_service_tool" = mongo ]; then
  :
else
...
dump_target_model="$(_god_resolve_command mongo "$catalog" backup 1 "$fixture_bin" '')"
if has_exact_line "$uri_target_model" $'DISPLAY\tmongosh "mongodb://mongo.internal:29017/<database>"' && \
   ...
   has_exact_line "$replica_target_model" $'DISPLAY\tmongosh --host mongo.internal --port 29017 --quiet --eval '\''rs.status()'\''' && \
   has_exact_line "$dump_target_model" $'DISPLAY\tmongodump --host mongo.internal --port 29017 --db <database> --out <output_directory>' && \
   ! printf '%s\n' "$replica_target_model" | LC_ALL=C grep -Fq '<host>'; then
  :
else
  fail 'MongoDB Target binds exact shell, URI, host-port, and Database Tool command forms'
fi
